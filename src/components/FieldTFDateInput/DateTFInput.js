...
class DateTFInputComponent extends Component {
  constructor(props) {
    super(props);
  ...
  onDatesChange(dates) {
    ...
    let endDateAsDate = new Date(startDateAsDate.getTime());
    endDateAsDate.setDate(endDateAsDate.getDate() + 1);
    //clearEndDate ? null : pickerEndDateToApiDate(unitType, endDate);
...
DateTFInputComponent.defaultProps = {
  className: null,
...
DateTFInputComponent.propTypes = {
  className: string,
  startDateId: string,
  endDateId: string,
...
export default injectIntl(DateTFInputComponent);
